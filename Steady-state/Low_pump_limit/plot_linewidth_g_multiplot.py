...
for i in range(3):
    for j in range(3):
        ...
        axs[i,j].text(0.01, axs[i,j].axes.get_ylim()[1]*1.1, '$g$={} THz'.format(g))
        

#augmentation
TITLE='{}-emitter. $\kappa$={:.2f} THz. $\gamma_A$={:.2f} THz. $\gamma_D$={:.2f} THz'.format(N_em,kappa,gamma,gammaD)
fig.suptitle(TITLE)
...
#save plot
plt.savefig('./plots/linewidth_g_multiplot_{}-emitter_pump-sweep_kap={}_gam={}_gam2={}.pdf'.format(N_em,kappa,gamma,gamma2))