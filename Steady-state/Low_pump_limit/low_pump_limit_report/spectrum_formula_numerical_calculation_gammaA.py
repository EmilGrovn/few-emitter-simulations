...
kappa=1
#kappa=0.1
gammaA_list=10**np.arange(-2,3+0.1,0.1)
#gammaA_list=10**np.arange(-5,3+0.1,0.1)
#gammaA_list=10**np.arange(-2,10+0.1,0.1)
#omega-list
dw=0.0001*g
...
linewidth_list=np.zeros(len(gammaA_list))    #linewidth
spectrum_list=[[]]*len(gammaA_list)          #spectrum
K1_list=np.zeros(len(gammaA_list))           #K1
K2_list=np.zeros(len(gammaA_list))           #K2
peakloc_list=np.zeros(len(gammaA_list))      #peak location
B21A12_list=np.zeros(len(gammaA_list))
B22A22_list=np.zeros(len(gammaA_list))
B21A11_list=np.zeros(len(gammaA_list))
B22A21_list=np.zeros(len(gammaA_list))
Koef_lamb1=np.zeros(len(gammaA_list))        #coeffiecient of 1st spectrum term
Koef_lamb2=np.zeros(len(gammaA_list))        #coeffiecient of 2nd spectrum term
lambda1_list=np.zeros(len(gammaA_list))
lambda2_list=np.zeros(len(gammaA_list))
for ele,gammaA in enumerate(gammaA_list):
    print('kappa',kappa)
...
np.savez('./data/spectrum_formula_linewidth_g={}_kap={}_gamD={}.npz'.format(g,kappa,gammaD),gammaA_list=gammaA_list,linewidth_list=linewidth_list)
print(linewidth_list-kappa)
#prediction
a0=-kappa/4
a1=-(gammaA_list+gammaA)/4
a2=a0-a1
b0=np.sqrt(a2**2-g**2)
...
for i in range(len(gammaA_list)):
    ...
    b0i=b0[i]
    if abs(a2i)<g:
        FWHM_linewidth[i]=(kappa+gammaA_list[i]+gammaA)/2
    elif a2i>g:
        #gammaA and gammaD dominates
        FWHM_linewidth[i]=2*np.sqrt(-a3i**2-b0i**2+np.sqrt(2*a3i**4+2*b0i**4))
        FWHM_linewidth_approx[i]=kappa+4*g**2/(gammaA_list[i]+gammaA-kappa)
    elif -a2i>g:
        #kappa dominates
        FWHM_linewidth[i]=2*np.sqrt(-a3i**2-b0i**2+np.sqrt(2*a3i**4+2*b0i**4))
        FWHM_linewidth_approx[i]=gammaA_list[i]+gammaA+4*g**2/(kappa-gammaA_list[i]-gammaA)   
...
ax1.loglog(gammaA_list,linewidth_list/(2*np.pi),'bo')
ax1.loglog(gammaA_list,(kappa)*np.ones(len(gammaA_list))/(2*np.pi),'b--')
ax1.loglog(gammaA_list,(gammaA+gammaA_list)*np.ones(len(gammaA_list))/(2*np.pi),'m--')
ax1.loglog(gammaA_list,2*np.absolute(np.real(lambda1_list))/(2*np.pi),'g')
ax1.loglog(gammaA_list,2*np.absolute(np.real(lambda2_list))/(2*np.pi),'r')
ax1.loglog(gammaA_list,FWHM_linewidth/(2*np.pi),'m.')
ax1.grid()
...
ax1.contourf(np.log10(gammaA_list)[:50], omega_list,(spectrum_list).transpose()[:,:50], 20, cmap='RdGy')
...
# bum=-3
# ax1.semilogx(gammaA_list[gammaA_list>bum],Cvec1,'m.')
# ax1.semilogx(gammaA_list[gammaA_list>bum],Cvec2,'b.')
# ax1.semilogx(gammaA_list[gammaA_list>bum],Koef_lamb1/np.absolute(Koef_lamb1),'m--')
# ax1.semilogx(gammaA_list[gammaA_list>bum],Koef_lamb2/np.absolute(Koef_lamb2),'b--')
# ax1.semilogx(gammaA_list[gammaA_list>bum],np.zeros(len(gammaA_list[gammaA_list>bum])),'k--')
...
ax1.semilogx(gammaA_list[gammaA_list>bum],Cvec1,'m.')
ax1.semilogx(gammaA_list[gammaA_list>bum],Cvec2,'b.')
ax1.semilogx(gammaA_list[gammaA_list>bum],Ko1/np.absolute(Ko1),'m--')
ax1.semilogx(gammaA_list[gammaA_list>bum],Ko2/np.absolute(Ko2),'b--')
ax1.semilogx(gammaA_list[gammaA_list>bum],np.zeros(len(gammaA_list[gammaA_list>bum])),'k--')
TITLE='{}-emitter. $g$={} ps$^-$$^1$. $\kappa$={} ps$^-$$^1$. $\gamma_A$={} ps$^-$$^1$.'.format(1,g,kappa,gammaA)
ax1.set(title=TITLE)
ax1.set(xlabel=r'$\gamma_A\,[\mathrm{ps^{-1}}]$')#,ylabel=r'$(|C2|-|C1|)/|C1|$')
...
print(Cvec1)
bum=-3
ax1.semilogx(gammaA_list[gammaA_list>bum],Ko1,'m.')
ax1.semilogx(gammaA_list[gammaA_list>bum],Ko2,'b.')
ax1.semilogx(gammaA_list[gammaA_list>bum],np.absolute(Ko2)-np.absolute(Ko1),'r.')
ax1.semilogx(gammaA_list[gammaA_list>bum],np.zeros(len(Ko1)),'k--')
TITLE='{}-emitter. $g$={} ps$^-$$^1$. $\kappa$={} ps$^-$$^1$. $\gamma_A$={} ps$^-$$^1$.'.format(1,g,kappa,gammaA)
ax1.set(title=TITLE)
ax1.set(xlabel=r'$\gamma_A\,[\mathrm{ps^{-1}}]$')#,ylabel=r'$(|C2|-|C1|)/|C1|$')
