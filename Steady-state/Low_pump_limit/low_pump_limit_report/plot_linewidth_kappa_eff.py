...
N_Hilbert=N_em+1
try:
    out_ss=np.load('./data/spectrum_eff_formula_linewidth_g={}_gamD={}_gamA={}.npz'.format(g,gammaD,gammaA))
...
for i in range(len(kappa_list)):
    ...
    if abs(a2i)<g:
        ...
        FWHM_linewidth_approx[i]=kappa_list_eff[i]+4*g**2/(gammaA+gammaD-kappa_list_eff[i])
    elif -a2i>g:
        #kappa dominates
        #FWHM_linewidth[i]=abs(2*a0[i]*(1-np.sqrt(1-g**2/a2i**2))+2*a1*(1+np.sqrt(1-g**2/a2i**2)))
...
plt.text(Q12/10, Max/2, '$a_2>g$')
plt.semilogx([Q23,Q23],[Min,Max],'k--')
plt.text(Q12+Q2len/10, Max/2, '$|a_2|<g$')
#plt.text(Q23/10, Max/1.3, '$|a_2|<g$')
plt.text(Q23*10, Max/2, '$-a_2>g$')
