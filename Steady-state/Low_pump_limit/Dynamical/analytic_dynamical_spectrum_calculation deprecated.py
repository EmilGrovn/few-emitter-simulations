...
# the Liouvillian matrix in a basis of EE, CC, EC-CE is
L=[[-gammaA,0,1j*g],[0,-kappa,-1j*g],[1j*2*g,-1j*2*g,-gammaCE]]
#print(L)
#find eigenvalues and vectors of L
Lambda,V=np.linalg.eig(L)
# print('eigenvalues',Lambda)
# print('eigenvectors','\n',V[:,0],'\n',V[:,1],'\n',V[:,2])
#We have checked that this is in accordance with our analytical calculation
#Now it is a matter of calculating the spectrum. 
# For this calculation we need to include GG in the basis B:
L=[[-gammaA,0,1j*g,0],[0,-kappa,-1j*g,0],[1j*2*g,-1j*2*g,-gammaCE,0],[gammaA,kappa,0,0]]
Lambda,V=np.linalg.eig(L)
# print('eigenvalues',Lambda)
# print('eigenvectors','\n',V[:,0],'\n',V[:,1],'\n',V[:,2],'\n',V[:,3])
invV=np.linalg.inv(V)#switches from basis B to basis eigenbasis
#we actually need to include the whole 9x9 eigenvalue problem
#basis B={EE,CC,EC,CE,GG,GE,EG,GC,CG}
