...
kappa_list_eff=kappa_list/N_em
N_Hilbert=N_em+1
try:
...
gammaR=4*g**2/(pump+gamma+gammaD+kappa_list_eff)
Reg1_linewidth=(3*pump+gamma+gammaD+kappa_list_eff)/2
Reg3_linewidth=kappa_list_eff/(nP_list_ss+1)
neList=neMatrix[:,0]
...
for i in range(len(kappa_list)):
    a2i=a2[i]
    if abs(a2i)<g:
        FWHM_linewidth[i]=(kappa_list_eff[i]+gammaD+gammaA)/2
        FWHM_linewidth_approx[i]=(kappa_list_eff[i]+gammaD+gammaA)/2
        FWHM_extended[i]=(kappa_list_eff[i]+gammaD+gammaA)/2
    elif a2i>g:
        #gammaA and gammaD dominates
        FWHM_linewidth[i]=abs(2*a0[i]*(1+np.sqrt(1-g**2/a2i**2))+2*a1*(1-np.sqrt(1-g**2/a2i**2)))
        FWHM_linewidth_approx[i]=kappa_list_eff[i]+4*g**2/(gammaD+gammaD-kappa_list_eff[i])
        FWHM_extended[i]=np.sqrt(-b0[i]**2+ np.sqrt(4*b0[i]**4+4*((a0[i]+a1)**2-b0[i]**2)**2))
    elif -a2i>g:
        #gammaA and gammaD dominates
        FWHM_linewidth[i]=abs(2*a0[i]*(1-np.sqrt(1-g**2/a2i**2))+2*a1*(1+np.sqrt(1-g**2/a2i**2)))
        FWHM_linewidth_approx[i]=gammaD+gammaA+4*g**2/(kappa_list_eff[i]-gammaD-gammaA)
        FWHM_extended[i]=np.sqrt(-b0[i]**2+ np.sqrt(4*b0[i]**4+4*((a0[i]+a1)**2-b0[i]**2)**2))   
...
ax1.legend(['Simulation','ST unmodified','ST modified','Rabi peak','Quenching','$\kappa$','P=0 analytical','P=0 analytical approx','full spectrum formula'],loc='upper left')
ax1.grid()
