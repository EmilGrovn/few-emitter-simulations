...
#
def gammaA_sweep_variable_NH(N_em,g,gamma2,gammaA_logmin,gammaA_logmax,kappa,pump):
    #gammaD-list
    dloggA=0.1
    loggA=np.arange(gammaA_logmin,gammaA_logmax+dloggA,dloggA)
    gammaA_List=10**(loggA)
    #perform gammaD-sweep
    Npoints=len(gammaA_List)
    ...
    for i,gamma in enumerate(gammaA_List):
        ...
        N_Hilbert_list[i]=N_Hilbert
    #save
    np.savez('./data/{}-emitter_gammaA-sweep_ss_NH_intelligent_g={}_gam2={}_kap={}_pump={}.npz'.format(N_em,g,gamma2,kappa,pump),g2=g2List,nP=npList,gammaA_List=gammaA_List,N_H_list=N_Hilbert_list,cav_conv=cav_capacity_occ,neMatrix=neMatrix)
    #tests
